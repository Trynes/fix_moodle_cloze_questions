...
/*-----------------------------------------------------------------------
*  Query the database to find all multianswer questions
------------------------------------------------------------------------*/
...
if (in_array('*', $checklist)) {
    //If you just want info on the duplicate entries (not verbose)...
    if(!$options['verbose']) {
...
function return_info($getinfo,$x,$options) {
	...
	//Or it has the wrong parent, or the sequence is '0',
	//or no sequence questions exist in the DB, or the usage is over 1,
...
if($options['questions']) {
    ...
    //I want a nice html document to view the results in...
	...
	foreach($plaintextGroups as $text => $group) {
		...
		if(!empty($getinfo)) {
			if (!empty($options['info'])) {
				//var_dump($getinfo);
				return_info($getinfo,$x,$options); //Just return the info, nothing more.
			}
			else if (!empty($options['fix'])) {
				//Run an info gathering mission first...
...
if(!empty($options['fix'])) {
    // Return info from the fix function that was run above...
	...
	// The info that would be returned here is actually returned in the function that was run before this.
    // Could put it here instead? Leaving this here just in case.
} else {
	echo "...To go through and store info for ALL multianswer questions, run: \$sudo -u wwwrun php ./fix_multianswer_questions_v3.php --questions=* --fix --verbose". "\n\n"
        ."...To go through and fix only duplicate sequence multianswer questions, don't use '--verbose'". "\n\n"
		."...To fix a specified question,  run: \$sudo -u wwwrun php ./fix_multianswer_questions_v3.php --questions=[type in questionid] --fix --verbose". "\n\n";
}
